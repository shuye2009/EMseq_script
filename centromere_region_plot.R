...
data_dir <- ifelse(LOCAL, file.path(local_home_dir, "h4h/data"), 
...
fiveP <- c(-50000, -500, -300, -300)
threeP <- c(50000, 500, 300, 300)
...
queryfiles <- list.files(path = data_dir, pattern = paste0(CONTEXT, ".bedGraph.gz$"))
...
if(TARGET %in% c("Centromere", "CpGIsland", "ERV", "ERVqPCR")){
  plot_region(
    ...
    transform = NA,
    stranded = FALSE,
    outPrefix = paste0("Methylation_level_in_", TARGET, "_", CONTEXT),
    Ylab = paste(CONTEXT, "methylation level"),
    rmOutlier = 0,
    nc = 5
  )
}
