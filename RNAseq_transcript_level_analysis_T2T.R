...
if(TRUE){
    ...
    outdir <- file.path(wd, "single_factor_analysis_transcript_level")
   ...
   # Extract results for all contrasts
    ...
    for(contrast_name in names(contrast_list)) {
        ...
        res_df_TE <- res_df %>%
            dplyr::filter(transcript_id %in% TE_df$transcript_id) %>%
            dplyr::left_join(TE_df[, c("transcript_id", "gene_name", "gene_id", "family_id", "class_id")], by = "transcript_id") %>%
            dplyr::distinct(transcript_id, .keep_all = TRUE)

        res_df_SAT <- res_df %>%
            dplyr::filter(transcript_id %in% SAT_df$transcript_id) %>%
            dplyr::distinct(transcript_id, .keep_all = TRUE)
        ...
        volcano_contrast <- create_volcano_plot(res_df_regular, contrast_name, 50, genome="hg38", label_col = "transcript_id", plot_gene_type = FALSE)
        ...
        volcano_contrast_SAT <- create_volcano_plot(res_df_SAT, paste0(contrast_name, "_SAT"), 50, genome="hg38", label_col = "transcript_id", plot_gene_type = FALSE)
