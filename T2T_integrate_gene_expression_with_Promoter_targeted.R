...
for(sample_name in sample_names){
    ...
    output_dir <- file.path(working_dir, "Integrate_gene_expression_with_Promoter")
    ...
    promoter <- read_tsv(file.path(working_dir, "all_diff_T2T_CHM13_promoters.tab")) %>%
        mutate(direction = case_when(
            meth.diff > 0 & pvalue < 0.25 ~ "Hypermethylated", 
            meth.diff < 0 & pvalue < 0.25 ~ "Hypomethylated",
            TRUE ~ "Not_significant")) %>%
        mutate(gene_name = sapply(strsplit(name, "_"), function(x) x[1])) %>%
        mutate(gene_id = sapply(strsplit(name, "_"), function(x) x[2]))
    ...
    p_cor <- ggplot(gene_promoter_overlap, aes(x = meth.diff, y = log2FoldChange)) +
        geom_point() +
