...
pdf("Methylation_level_distribution_by_CONTEXT_limit.pdf")
for(CONTEXT in c("CpG", "CHG", "CHH")){
  ...
  p1 <- ggplot2::ggplot(bg_df) +
    geom_density(aes(x=methylation_level, color=treatment), adjust = 5, linewidth = 1.2) +
...
pdf("Methylation_level_distribution_by_TREATMENT_limit.pdf")
for(t in c("R172K_dox_IR_S3", "R172K_dox_NIR_S4", "WT_dox_IR_S1", "WT_dox_NIR_S2")){
  ...
  p1 <- ggplot2::ggplot(bg_df) +
    geom_density(aes(x=methylation_level, color=context), adjust = 5, linewidth = 1.2) +
    ggtitle(paste0(t, ">0")) +
    theme_classic()
  print(p1)
