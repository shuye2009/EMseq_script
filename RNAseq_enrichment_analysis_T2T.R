...
genome <- "T2T_CHN13v2.0"
GSEA <- FALSE # set to TRUE if GSEA is to be performed
...
SAT_gtf <- "C:/PROJECTS/resource/T2T_CHM13/chm13v2.0_censat_v2.1.gtf"
SAT_gr <- plyranges::read_gff(SAT_gtf)
SAT_df <- data.frame(gene_id = SAT_gr$gene_id, 
                    gene_name = SAT_gr$gene_name, 
                    transcript_id = SAT_gr$transcript_id)
...
if(TRUE){
    # Load count data
    wd <- "C:/PROJECTS/Shane/Harding_250611/T2T_CHM13/RNAseq/deseq2"
    setwd(wd)
    rsem_count <- read.delim("rsem_gene_level_counts.tsv", 
                            header = TRUE, row.names = 1)
                            # Average counts for duplicated GENENAME using group_by and summarize
   ...
   # Extract results for all contrasts
    ...
    for(contrast_name in names(contrast_list)) {
        ...
        res_df_TE <- res_df %>%
            ...
            dplyr::left_join(TE_df[, c("gene_name", "gene_id", "family_id", "class_id")], by = "gene_name") %>%
            dplyr::distinct(gene_name, .keep_all = TRUE) %>%
            mutate(direction = ifelse(ifelse(padj < 0.05 & log2FoldChange > 0, "up", "down"), "noChange")) %>%
            mutate(direction = factor(direction, levels = c("up", "down", "noChange"))) %>%
            arrange(direction, padj)

        res_df_SAT <- res_df %>%
            dplyr::filter(gene_name %in% SAT_df$gene_name) %>%
            dplyr::distinct(gene_name, .keep_all = TRUE) %>%
            mutate(direction = ifelse(ifelse(padj < 0.05 & log2FoldChange > 0, "up", "down"), "noChange")) %>%
            mutate(direction = factor(direction, levels = c("up", "down", "noChange"))) %>%
            arrange(direction, padj)

        res_df_regular <- res_df %>%
            dplyr::filter(!gene_name %in% TE_df$gene_name) %>%
            dplyr::filter(!gene_name %in% SAT_df$gene_name) %>%
            mutate(direction = ifelse(ifelse(padj < 0.05 & log2FoldChange > 0, "up", "down"), "noChange")) %>%
            mutate(direction = factor(direction, levels = c("up", "down", "noChange"))) %>%
        ...
        # Save results
        write.csv(res_df_regular, paste0(contrast_name, "_DESeq2_results_regular.csv"), row.names = FALSE)
        write.csv(res_df_TE, paste0(contrast_name, "_DESeq2_results_TE.csv"), row.names = FALSE)
        write.csv(res_df_SAT, paste0(contrast_name, "_DESeq2_results_SAT.csv"), row.names = FALSE)
        cat(paste("\nSaved:", contrast_name))
        ...
        volcano_contrast <- create_volcano_plot(res_df_regular, contrast_name, 50, genome="hg38", plot_gene_type = TRUE)
        volcano_contrast_TE <- create_volcano_plot(res_df_TE, paste0(contrast_name, "_TE"), 50, genome="hg38", plot_gene_type = FALSE)
        volcano_contrast_SAT <- create_volcano_plot(res_df_SAT, paste0(contrast_name, "_SAT"), 50, genome="hg38", plot_gene_type = FALSE)
        # GSEA plot
        if(GSEA){
            create_gsea_plot(res_df_regular, contrast_name)
        }
    } 
...
if(TRUE){
    ...
    data_dir <- "C:/PROJECTS/Shane/Harding_250611/T2T_CHM13/RNAseq/deseq2"
    working_dir <- "single_factor_analysis_gene_level"
    ...
    outdir <- file.path(data_dir, working_dir)
    if(!dir.exists(outdir)) dir.create(outdir)
    ...
    plot_gene_list(cenpa_list, "cenpa_genes", NIR_10Gy24h_abundance_matrix, outdir, "10Gy24h_vs_NIR")

    plot_gene_list(dnmt_list, "dnmt_genes", abundance_matrix, data_dir, working_dir, width = 12, height = 8)
    plot_gene_list(dnmt_list, "dnmt_genes_10Gy", NIR_10Gy_abundance_matrix, data_dir, working_dir, width = 12, height = 8)
    plot_gene_list(dnmt_list, "dnmt_genes_2Gy", NIR_2Gy_abundance_matrix, data_dir, working_dir, width = 12, height = 8)
    ...
    ggsave(file.path(data_dir, working_dir, paste0("dnmt_genes_10Gy_lineplot.pdf")), 
